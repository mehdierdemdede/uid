...
@section('title', $news->localizedTitle() . ' — UID Bosna Hersek')
    ...
    <div class="mx-auto max-w-4xl px-4 md:px-8">
        ...
        @if($news->image_path)
            <div class="mb-10 overflow-hidden rounded-2xl shadow-md">
                <img src="/storage/{{ $news->image_path }}" alt="{{ $news->localizedTitle() }}" class="h-auto w-full max-h-[500px] object-cover">
            </div>
        @endif

        <h1 class="mb-4 text-3xl font-bold text-slate-900 md:text-5xl">{{ $news->localizedTitle() }}</h1>
