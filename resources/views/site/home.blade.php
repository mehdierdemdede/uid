...
@section('title', __('Ana Sayfa').' — UID Bosna Hersek')
...
@section('content')
                ...
                <div class="flex gap-4">
                    ...
                    <div class="space-y-3">
                        <h1 class="text-4xl font-bold leading-tight md:text-5xl lg:text-6xl">
                            {{ __('UID, yurt dışındaki Türk kökenli kişilerin siyasi') }}
                        </h1>
                        <p class="max-w-xl text-base text-white/90 md:text-xl">
                            {{ __('sosyal ve kültürel gelişimini, Bosna Hersek odaklı faaliyetlerle desteklemektedir.') }}
                        </p>
                    </div>
                </div>

                <div class="flex flex-wrap gap-3 pt-4">
                    <a href="{{ t_route('news.index') }}" class="inline-flex items-center gap-2 bg-white px-8 py-3 text-sm font-semibold text-slate-800 transition hover:bg-slate-100">
                        {{ __('Haberler') }}
                        <span aria-hidden="true">&gt;</span>
                    </a>
                    <a href="{{ t_route('membership.create') }}" class="inline-flex items-center gap-2 bg-uid-blue px-8 py-3 text-sm font-semibold text-white transition hover:bg-uid-blue/90">
                        {{ __('Üye ol') }}
                        <span aria-hidden="true">&gt;</span>
                    </a>
                </div>
    ...
    <section class="bg-white bg-cover bg-center py-16 md:py-20" style="background-image: url('{{ asset('images/bg_weiss.jpg') }}');">
        <div class="mx-auto max-w-5xl px-4 md:px-6 lg:px-8">
            <div class="mb-10 flex items-center gap-4">
                ...
                <h2 class="shrink-0 text-sm font-medium text-slate-500">{{ __('Güncel') }}</h2>
                <span class="h-px flex-1 bg-slate-400" aria-hidden="true"></span>
            </div>
            ...
            <div class="overflow-hidden border border-slate-200 bg-white shadow">
                <a href="https://www.youtube.com" target="_blank" rel="noopener noreferrer" class="group relative block aspect-video bg-slate-800">
                    ...
                    <span class="absolute left-1/2 top-1/2 flex h-20 w-20 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-uid-blue/90 text-white shadow-xl ring-8 ring-uid-blue/35 transition group-hover:scale-105">
                        ...
                        <span class="sr-only">{{ __('Videoyu aç') }}</span>
                    </span>
                    <div class="absolute bottom-4 left-4 bg-white/95 px-4 py-3 shadow">
                        ...
                        <p class="text-lg text-uid-navy/80">{{ __('Genel Başkan') }}</p>
                    </div>
                </a>
            </div>
        </div>
    ...
    <section class="bg-cover bg-center py-16 text-white md:py-24" style="background-image: url('{{ asset('images/bg_blue.png') }}');">
        <div class="mx-auto grid max-w-[1400px] items-center gap-8 px-4 md:grid-cols-[1.2fr_1fr] md:px-8 lg:px-10">
            <div>
                <h2 class="mb-6 text-4xl font-bold md:text-5xl">{{ __('Misyonumuz') }}</h2>
                <div class="space-y-4 text-base leading-relaxed text-white/95 md:text-lg">
                    <p>
                        {{ __("2004 yılında Köln'de UETD - Union of European Turkish Democrats ismi altında kurulan ve tüm dünyadan gelen taleplere cevap verebilmek için, 20 Mayıs 2018'de Bosna Hersek'te gerçekleşen 6. Olağan Genel Kurulu ile ismini UID - Union of International Democrats olarak değiştirerek bir dünya teşkilatına evrilmiştir.") }}
                    </p>
                    <p>
                        {{ __('UID Türklerin ve kardeş toplulukların ilişkilerini küresel çapta güçlendirerek ekonomik, sosyal ve kültürel olarak daha yakın ilişkiler tesis etmeyi hedeflemektedir.') }}
                    </p>
                </div>
            </div>
            <div class="flex items-center justify-end px-4">
                <img src="{{ asset('images/Weltkarte.png') }}" alt="{{ __('Misyon Dünya Haritası') }}" class="h-auto w-full max-w-[520px] object-contain opacity-95">
            </div>
        </div>
    </section>
    ...
    <section class="bg-cover bg-center py-16 text-white md:py-24" style="background-image: url('{{ asset('images/bg_turkis.jpg') }}');">
        <div class="mx-auto grid max-w-[1400px] items-center gap-8 px-4 md:grid-cols-[1.2fr_1fr] md:px-8 lg:px-10">
            <div>
                <h2 class="mb-6 text-4xl font-bold md:text-5xl">{{ __('Vizyonumuz') }}</h2>
                <div class="space-y-4 text-base leading-relaxed text-white/95 md:text-lg">
                    <p>
                        {{ __('Vizyonumuz, farklı uluslara, kültürlere veya dinlere mensup insanların toplumsal yaşamın her alanına katılım sağlayan ve toplumu şekillendiren, eşit haklara sahip bireyler olmasıdır.') }}
                    </p>
                    <p>
                        {{ __('UID bu vizyona ulaşmak için, tüm dünyada Türklerin ve kardeş toplulukların yaşadıkları toplumlara uyumunu destekleyen çalışmalar yürütür; farklılıklara saygı ve karşılıklı anlayışı temel alır.') }}
                    </p>
                </div>
            </div>
            <div class="flex items-center justify-end px-4">
                <img src="{{ asset('images/Lampe-Idee.png') }}" alt="{{ __('Vizyon İkonu') }}" class="h-auto w-full max-w-[420px] object-contain opacity-95">
            </div>
        </div>
    </section>
@endsection
